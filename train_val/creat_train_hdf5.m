...
load train_val_data_label TraData_add TraLab

% % WRITING TO HDF5
filename = 'Train_data_label.h5';

num_total_samples = size( TraData_add, 2 );
LabDim = size( TraLab, 1 );

% to simulate data being read from disk / generated etc.
data_disk = zeros( 192, 1, 1, num_total_samples );
data_disk( :, 1, 1, : ) = TraData_add;
...
data_rd = h5read(filename, '/data', [1 1 1 1000], [192, 1, 1, 1000] );
label_rd = h5read(filename, '/label', [1 1000], [LabDim, 1000] );
fprintf( 'Testing ...\n' );
try 
