...
load train_val_data_label ValData_add ValLab

% % WRITING TO HDF5
filename = 'Val_data_label.h5';

num_total_samples = size( ValData_add, 2 );
LabDim = size( ValLab, 1 );

% to simulate data being read from disk / generated etc.
data_disk = zeros( 192, 1, 1, num_total_samples);
data_disk( :, 1, 1, : ) = ValData_add;
label_disk = zeros( LabDim, num_total_samples ); 
label_disk( :, : ) = ValLab;
...
data_rd = h5read( filename, '/data', [1 1 1 10], [192, 1, 1, 10] );
label_rd = h5read( filename, '/label', [1 10], [LabDim, 10] );
fprintf( 'Testing ...\n' );
try 
  assert( isequal( data_rd, single( data_disk( :, :, :, 10:19 ) ) ), 'Data do not match' );
  assert( isequal( label_rd, single( label_disk( :, 10:19 ) ) ), 'Labels do not match' );
